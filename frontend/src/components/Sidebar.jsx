    ...
    <aside className="fixed top-0 left-0 h-screen w-64 bg-gray-700 border-r border-gray-200 flex flex-col justify-between">
      <div>
        <div className="py-4 px-6 flex flex-col items-center">
          <img src={logo} alt="Timeless Soft" className="w-40 h-auto" />
        ...
        <nav className="px-6 py-4 space-y-2">
          ...
          {isAdmin && (
            <>
              <SidebarLink to="/enter-hours" icon="🕒" label="Stunden" />
              <SidebarLink to="/users" icon="👥" label="Angestellte" />
              <SidebarLink to="/requests" icon="📦" label="Anfragen" />
            </>
          )}
