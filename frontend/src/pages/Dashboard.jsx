...
const Dashboard = () => {
    ...
    const columns = [
        ...
        },
        {
            key: "year",
            label: "Jahr",
            format: (_, val) => new Date(val.startDate).getFullYear(),
        },
            ...
            <div className="py-12">
                    ...
                    <DataTable
                        ...
                        renderActions={(row) =>
                            row.status === "pending" ? (
                                <button
                                    className="bg-red-500 text-white px-2 py-1 rounded"
                                    onClick={() => setDeleteSickId(row._id)}
                                >
                                    Löschen
                                </button>
                            ) : (
                ...
                {deleteSickId && (
                    <ConfirmModal
                        ...
                        onConfirm={async () => {
                            await api.delete(
                                `/dashboard/sickness/${deleteSickId}`
                            );
                            setSicknesses((prev) =>
                                prev.filter((s) => s._id !== deleteSickId)
                            );
